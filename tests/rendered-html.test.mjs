...
test("server-renders the OGB comic storefront", async () => {
  ...
  assert.match(html, /Read issue #1/i);
...
test("ships the complete 24-page reader and final assets", async () => {
  const [readerSource, packageJson, pageFiles] = await Promise.all([
    readFile(new URL("../app/ComicExperience.tsx", import.meta.url), "utf8"),
